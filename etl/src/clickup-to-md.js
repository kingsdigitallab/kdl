...
class ClickUpToMarkdown {
	...
	async fetchClickUpData(customItemId) {
		try {
			...
			while (!isLastPage) {
				...
				if (taskResponse.data.last_page || taskResponse.data.tasks.length === 0) {
					isLastPage = true;
					break;
				}
			...
			if (error.response) {
				throw new Error(`ClickUp API error: ${error.response.status} ${error.response.statusText}`);
			}
			throw error;
		}
	}
	...
	async convert(data) {
		...
		for (const project of projects) {
			let name = this.getCustomFieldValue(project, "Project name") || project.name;
			...
			const projectStartDate = this.getCustomFieldValue(project, "Project start date");
			...
			const spaceStatus = spaces.find((space) => space.id === project.space.id)?.name?.trim();
			...
			const descriptionLen = existingContent?.split("---")[2]?.trim().length || 0;
			...
			const frontmatter = {
				...
				slug: slug,
				foundingDate,
				dissolutionDate: this.convertDate(this.getCustomFieldValue(project, "Project end date")),
				creativeWorkStatus,
			...
			if (urls.length > 0) {
				frontmatter.urls = [...(frontmatter?.urls || []), ...urls].reduce((acc, curr) => {
	...
	async getAndConvertFunders(data) {
		const funderField = data.custom_fields.find((field) => field.name === "Funder(s)");

		if (!funderField || !funderField.value) return [];

		const funders = funderField.value.map((funder) => {
			const found = funderField.type_config.options.find((opt) => opt.id === funder);
		...
		for (const funder of funders) {
			const funderOutputFile = path.join(this.outputPath, `../about/organisations/${funder.slug}.md`);
	...
	async getAndConvertDepartments(data) {
		...
		const deptsField = data.custom_fields.find((field) => field.name === "Department(s)");

		if (deptsField) {
			deptsField.value.forEach((dept) => {
				const found = deptsField.type_config.options.find((opt) => opt.id === dept);
		...
		const partnersField = data.custom_fields.find((field) => field.name === "Partner organisation(s)");

		if (partnersField) {
			partnersField.value.forEach((partner) => {
				const found = partnersField.type_config.options.find((opt) => opt.id === partner);
		...
		for (const department of departments) {
			const departmentOutputFile = path.join(this.outputPath, `../about/organisations/${department.slug}.md`);
			...
			const formattedDepartmentContent = await this.formatMarkdown(departmentContent);
	...
	getMembers(data) {
		...
		const researchersField = data.custom_fields.find((field) => field.name === "Researchers");
		...
		return members.filter((member, index, self) => {
			return index === self.findIndex((t) => t.slug === member.slug && t.roleName === member.roleName);
		});
	}
	 ...
	 */
	getKeywords(data) {
		const themeField = data.custom_fields.find((field) => field.name === "Theme");
		...
		return themeField.type_config.options
			...
			.map((opt) => ({
				name: opt.label,
				slug: opt.label === "Machine Learning and AI" ? "machine-learning-ai" : this.slugify(opt.label),
			}));
	}
	...
	getSLADates(data) {
		...
		return {
			start: startDate ? new Date(parseInt(startDate)).toISOString().split("T")[0] : null,
			end: endDate ? new Date(parseInt(endDate)).toISOString().split("T")[0] : null,
		};
	}
